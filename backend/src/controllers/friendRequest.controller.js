...
const areFriends = async (email1, email2) => {
    ...
    console.log("Pool in areFriends:", pool);
...
export const acceptRequest = async (req, res) => {
    ...
    try {
        ...
        const pool = await getSqlPool(); // mặc định là server trung tâm (server1)

        // Truy vấn thông tin lời mời từ SQL (dựa vào requestId)
        const requestResult = await pool.request()
            .input("requestId", sql.Int, requestId)
            .query(`SELECT * FROM Friend_requests WHERE Request_id = @requestId`);
        ...
        // Lấy server hai bên từ MongoDB
        const users = await Email.find({ email: { $in: [Sender_email, Receiver_email] } }).select("email server");
        ...
        // Gọi stored procedure xử lý accept request (chạy tại server mặc định)
        await pool.request()
            .input("SenderEmail", sql.VarChar, Sender_email)
            .input("ReceiverEmail", sql.VarChar, Receiver_email)
